...
by simplification_classification tax_department export_import, sort: egen valeur_totale_par_marchandise=total(valeur)		
...
bysort year simplification_classification export_import tax_department: keep if _n==1

gen inflation=.
sort simplification_classification export_import tax_department year
bys simplification_classification export_import tax_department: replace inflation=100*prix_pondere_annuel[_n]/prix_pondere_annuel[_n-1]

gen IPC=.
bys simplification_classification export_import tax_department: replace IPC=100*prix_pondere_annuel[_n]/prix_pondere_annuel[1]
gen panvar = simplification_classification + export_import + tax_department
...
* Graphique juste IPC
twoway (line IPC year if tax_department=="Bordeaux" & simplification_classification =="acier" & export_import=="Exports")

* Graphique avec inflation
twoway (line IPC year if tax_department=="Bordeaux" & simplification_classification =="acier" & export_import=="Exports") (line inflation year if tax_department=="Bordeaux" & simplification_classification =="acier" & export_import=="Exports")


* En cas de valeur aberrante, supprimer la ligne posant problème, exemple : 
 drop if simplification_classification=="acier" & year==1724 & tax_department=="Bordeaux" & export_import=="Imports"
...
bys simplification_classification tax_department export_import : egen nbr_annees=count(value_unit_converti) 
egen nbr_annees_max=max(nbr_annees) 
bys simplification_classification tax_department export_import : drop if nbr_annees < nbr_annees_max
...
gen p0q0=.
bys simplification_classification export_import tax_department: replace p0q0=value_unit_pondere[1]*quantite_echangee[1] 

gen pnqn=.
bys simplification_classification export_import tax_department: replace pnqn=value_unit_pondere[_n]*quantite_echangee[_n] 
 
gen p0qn=.
bys simplification_classification export_import tax_department: replace p0qn=value_unit_pondere[1]*quantite_echangee[_n] 
 
gen pnq0=.
bys simplification_classification export_import tax_department: replace pnq0=value_unit_pondere[_n]*quantite_echangee[1] 

* Calcul indices de Laspeyres, Paasche et Fisher

gen laspeyres=. 
bys simplification_classification export_import tax_department: replace laspeyres=sum(pnq0)/sum(p0q0)

gen paasche=.
bys simplification_classification export_import tax_department: replace paasche=sum(pnqn)/sum(p0qn)

gen fisher=.
bys simplification_classification export_import tax_department: replace fisher=sqrt(laspeyres*paasche) 
...
* Comparaisons graphiques, exemple : 
	...
	twoway (line laspeyres year if tax_department=="La Rochelle" & simplification_classification =="sel" & export_import=="Exports")
	twoway (line paasche year if tax_department=="La Rochelle" & simplification_classification =="sel" & export_import=="Exports")
	twoway (line fisher year if tax_department=="La Rochelle" & simplification_classification =="sel" & export_import=="Exports")
...
* Choisir une tax_department
encode tax_department, gen(tax_department_num)
bysort tax_department_num: drop if _N<=100
codebook tax_department
keep if tax_department=="nom de la tax_department" 
