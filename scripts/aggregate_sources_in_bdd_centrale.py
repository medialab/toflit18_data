...
def normalize_year(year):
    ...
    if not m:
        ...
        if nb < 2 or nb > 14:
            raise Exception(
                'toflit18.republican_calendar.normalizeYear: invalid year %s.' % nb)
        return 1792 + nb


def empty_value(v): return v == "0" or v == "." or v == "" or v == "?"
...
def add_calculated_fields_to_line(d):
    ...
    if (
        ...
        (not(empty_value(d["value"]) and empty_value(
            d["quantity"]) and empty_value(d["value_per_unit"])))
    ):
...
def aggregate_sources_in_bdd_centrale(with_calculated_values=False):
    ...
    with open('../csv_sources_schema.json', 'r', encoding='utf8') as f:
        ...
        number_fields = [f['name']
                         for f in toflit18_flow_schema['fields'] if f["type"] == "number"]
    # headers = []
                    ...
                    if ext == "csv":
                        ...
                        with open(filepath, "r", encoding="utf-8") as source_file:
                            ...
                            if r.fieldnames != ordered_headers:
                                print("%s file does not respect the format" %
                                      filepath)
                                print(r.fieldnames)
                                exit(1)
                            for line in r:
                                ...
                                if with_calculated_values:
                                    ...
                                    # republican calendar back to current
                                    year = normalize_year(line['year'])
                                    if (line['source_type'] == "Objet Général" and year <= 1780 and year >= 1754) or line['source_type'] == "Résumé" or line['source_type'] == "National toutes directions tous partenaires":
                                        line['best_guess_national_prodxpart'] = 1
                                        best_guess_year_index['best_guess_national_prodxpart'].add(
                                            year)
                                    # best_guess_national_product
                                    if (line['source_type'] == "Objet Général" and year <= 1780 and year >= 1754) or line['source_type'] == "Résumé" or line['source_type'] == "National toutes directions tous partenaires" or (line['source_type'] == "Tableau des quantités" and (year >= 1822 or year==1793)):
                                    ...
                                    # best_guess_national
                                    if line['source_type'] == "Tableau Général" or line['source_type'] == "Résumé" or (line['source_type'] == "Tableau des quantités" and (year >= 1822 or year==1793)) :
