...
Data_Marseille <- bdd_courante %>%
  ...
  ### On selectionne uniquement le port de La Marseille
  filter(customs_region == "Marseille") %>%
...
for (prod in levels(Data_Marseille$product_simplification)) {
  
  if (sum(Data_Marseille$product_simplification == prod
          & Data_Marseille$export_import == "Imports"
          & Data_Marseille$best_unit_metric == T) > 20) {
    
    Data = subset(Data_Marseille,  product_simplification == prod 
...
for (prod in levels(Data_Marseille$product_simplification)) {
  ...
  Data_imports = subset(Data_Marseille,  product_simplification == prod 
                & export_import == "Imports"
                & best_unit_metric == T)
  
  Data_exports = subset(Data_Marseille,  product_simplification == prod 
...
Marseille_trans <- rtCreateTrans(trans_df = subset(Data_Marseille, best_unit_metric == T 
                                                  & trans_number > 1
                                                  & export_import == "Imports"
                                                  & outliers == F),
