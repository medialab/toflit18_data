...
**Et ne pas partir de goods_simpl_classification (ou alors mettre barre pour éliminer les noms simplifiés plus haute qu'à 10
 ...
 collapse (sum) value,by(grouping_classification exportsimports year goods_simpl_classification)
 ...
 * count if abs(résiduImp)>8
 ...
 regress ln_somme_value i.goods_simpl_classification_num i.year i.grouping_classification_num if exportsimports=="Exports"
...
merge 1:m year goods_simpl_classification exportsimports  grouping_classification ///
					using "$dir/Données Stata/bdd courante.dta"
 

 keep if résidu_élevé==1
drop if résiduImp>=`plancher' & exportsimports=="Imports"
drop if résiduExp>=`plancher' & exportsimports=="Exports"
drop if résiduExp==. & résiduImp==.
 
 
sort year exportsimports goods_simpl_classification grouping_classification
 
 
 
keep year exportsimports goods_simpl_classification grouping_classification ///
			value somme_value  somme_value_predExp somme_value_predImp ///
			direction ///
			numrodeligne sourcepath sheet marchandises pays ///

order year exportsimports goods_simpl_classification grouping_classification ///
			value somme_value  somme_value_predExp somme_value_predImp ///
			direction ///
			numrodeligne sourcepath sheet marchandises pays ///
