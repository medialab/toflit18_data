...
collapse (sum) value, by(year export_import customs_region product_canada product_sitc product_beaver source_type country_grouping)
*fillin year customs_region product_canada country_grouping
replace value=. if value==0

gen commerce_national = 1 if (source_type=="Objet Général" & year<=1786) | (source_type=="Résumé" & year <= 1790) | source_type=="National toutes customs_regions tous partenaires"

save blink.dta, replace
keep if year==1750 & source_type=="National toutes customs_regions tous partenaires"
...
erase blink.dta
replace commerce_national=0 if customs_region!=""

gen commerce_local = 1 if source_type=="Local" & year!=1750 | source_type=="National toutes customs_regions tous partenaires" | ////
			(source_type=="National toutes customs_regions partenaires manquants" & year==1788 & country_grouping=="Outre-mers") | ////
			(source_type=="National toutes customs_regions partenaires manquants" & year==1789 & country_grouping=="Pas Outre-mers")

**Nous n'avons pas les importations depuis les colonies par customs_region de ferme pour 1789
...
if "`product_dinteret'"=="product_canada" {
	...
	collapse (sum) value, by(year export_import customs_region product_canada source_type country_grouping commerce_national commerce_local) 
}
...
if "`product_dinteret'"=="product_sitc" {
	...
	collapse (sum) value, by(year export_import customs_region product_sitc source_type country_grouping commerce_national commerce_local) 
}
...
if "`product_dinteret'"=="product_beaver" {
	...
	*replace product_sitc="others" if product_sitc!="6a" & product_sitc!="0b"
	...
	collapse (sum) value, by(year export_import customs_region product_beaver source_type country_grouping commerce_national commerce_local) 
}
...
foreach customs_region in Nantes Marseille Bayonne Bordeaux Rouen Rennes "La Rochelle"  {

	local name = subinstr("`customs_region'"," ","_",.)

	local conditions_options commerce_local==1 & customs_region=="`customs_region'" & country_grouping=="Outre-mers", msize(small) cmissing(n) 

	twoway `lignes_du_graph' ///
		`legende' ///
	ytitle("Millions of livres tournois") title("`customs_region', Atlantic trade") ///
	xscale(range(1715 1790)) xlabel(1720 (10) 1790) ///
	name("`name'_OM", replace)
	graph export "`product_dinteret'_`name'_OM.png", replace

	local conditions_options commerce_local==1 & customs_region=="`customs_region'" & country_grouping=="Pas Outre-mers", msize(small) cmissing(n) 
	
	twoway `lignes_du_graph' ///
		`legende' ///
		ytitle("Millions of livres tournois") title("`customs_region', Not Atlantic trade") ///
		xscale(range(1715 1790)) xlabel(1720 (10) 1790) ///
...
collapse (sum) value, by(export_import year customs_region  `product_dinteret' source_type commerce_national commerce_local)
...
foreach customs_region in Nantes Marseille Bayonne Bordeaux Rouen Rennes "La Rochelle"  {
	...
	local conditions_options commerce_local==1 & customs_region=="`customs_region'", msize(small) cmissing(n) 


	twoway `lignes_du_graph' ///
		`legende' ///
		xscale(range(1715 1790)) xlabel(1720 (10) 1790) ///
		ytitle("Millions of livres tournois") title("`customs_region'") ///
		name("`name'", replace)
