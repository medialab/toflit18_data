...
bys natlocal importexport : egen total_trade = total(value)

*** aggregate by: country, importexport, year
collapse (sum) value, by ( natlocal total_trade importexport grains_num grains)
*** ratio of grains on total import or export
bys importexport grains_num  natlocal: generate g_ratio = (value/total_trade)*100
...
reshape wide g_ratio, i( grains_num natlocal) j(importexport)
rename g_ratio0 import
rename g_ratio1 export

keep if natlocal=="National"
graph pie import, over(grains_num) plabel(_all percent) 
graph pie export, over(grains_num) plabel(_all percent)
