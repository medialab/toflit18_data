...
sort year importexport value_inclusive geography grains_num partner_grouping product_simplification
quietly by year importexport value_inclusive geography grains_num partner_grouping product_simplification:  gen dup = cond(_N==1,0,_n)
drop if dup>1 
...
keep if  source_type_grains=="National" | source_type_grains=="Local" 
...
collapse (sum) value_inclusive, by (year geography importexport partner_grouping source_type_grains)

*** reshape
*reshape wide value_inclusive total_trade country_ratio, i(year partner_grouping source_type_encode) j(importexport)
reshape wide value_inclusive, i(year partner_grouping geography source_type_grains) j(importexport)
...
bys period geography source_type_grains partner_grouping : egen avnxport = mean(nxport)
...
****************************************ALFANI O'GRADA DB**************
merge m:1 year partner_grouping using "Données Stata/alfaniograda.dta"

gen differential=famine-france


encode partner_grouping, generate(partner) label(partner_grouping)
