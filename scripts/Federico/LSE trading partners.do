...
sort year importexport value_inclusive geography grains_num pays_simplification marchandises_simplification
quietly by year importexport value_inclusive geography grains_num pays_simplification marchandises_simplification:  gen dup = cond(_N==1,0,_n)
drop if dup>1 
...
*adjust 1749, 1751, 1777, 1789 and double accounting in order to keep only single values from series "Local" and "National toutes directions partenaires manquants"
sort year importexport value_inclusive geography grains_num pays_grouping pays_grouping
quietly by year importexport value_inclusive geography grains_num pays_grouping pays_grouping:  gen dup = cond(_N==1,0,_n)
drop if dup>1 
clonevar sourcetype_grains=sourcetype
replace sourcetype_grains="National" if sourcetype=="Résumé"
replace sourcetype_grains="National" if sourcetype=="Tableau des quantités"
