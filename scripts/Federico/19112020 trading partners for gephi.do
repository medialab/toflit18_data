...
***clean partner_grouping and create OTHERS OR UKNOWN and IBERIA

replace partner_grouping="USA" if partner_grouping=="États-Unis d'Amérique"
*** Empire
replace partner_grouping="Empire Suisse Flandre" if partner_grouping=="Flandre et autres états de l'Empereur"
replace partner_grouping="Empire Suisse Flandre" if partner_grouping=="Allemagne"
replace partner_grouping="Empire Suisse Flandre" if partner_grouping=="Suisse"
*** others
replace partner_grouping="others" if partner_grouping=="????"
replace partner_grouping="others" if partner_grouping=="Asie"
replace partner_grouping="others" if partner_grouping=="Inconnu"
replace partner_grouping="others" if partner_grouping=="Divers"
replace partner_grouping="others" if partner_grouping=="unknown"
***Americas other than US
replace partner_grouping="Americas" if partner_grouping=="Outre-mers"
replace partner_grouping="Americas" if partner_grouping=="Amériques"
***Iberia
replace partner_grouping="Iberia" if partner_grouping=="Espagne"
replace partner_grouping="Iberia" if partner_grouping=="Portugal"
*** deal with Africa
replace partner_grouping="MENA" if partner_grouping=="Levant et Barbarie"
replace partner_grouping="MENA" if partner_grouping=="Afrique"


***create sum of locals and variables giving number of directions (same for imports and exports in every year except 1789)
collapse (sum) value, by(year importexport natlocal partner_grouping )
...
collapse (sum) value, by(year importexport n_directions partner_grouping)
...
drop guess_value  n_direction
**merge
merge 1:1 year importexport partner_grouping using local_temp.dta
drop _merge
erase national_temp.dta 
...
*** aggregate by: country, importexport, year GUILLAUME PLEASE CHECK THIS MAKES SENSE
collapse (mean) value guess_value, by (period importexport partner_grouping)

***simplify in order for gephi to understand weights
gen weight=value
replace weight=guess_value if period==1
drop if weight==0
drop if weight==.

***create attribute import+export
bys partner_grouping period: egen Attribute=sum(weight)
***create edge table
gen Source=partner_grouping
replace Source="France" if importexport==1
gen Target=partner_grouping
replace Target="France" if importexport==0
drop partner_grouping
gen Label="import"
replace Label="export" if importexport==1
drop importexport value guess_value
rename period Timestamps
gen id = _n
***export as csv
export delimited using "Données Stata/edges1.csv",     replace
...
sort Target
quietly by Target:  gen dup = cond(_N==1,0,_n)
drop  Timestamps weight Source Label
rename Target Id
drop if dup>1
drop dup
gen Label=Id
**export as csv
export delimited using "Données Stata/nodes.csv",    replace
