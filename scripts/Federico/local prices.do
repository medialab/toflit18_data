...
replace quantity_unit_ortho="chargesgrandes" if quantity_unit_ortho=="charges" & value_unit>=28
replace q_conv=0.4895*300 if quantity_unit_ortho=="chargesgrandes" & value_unit>=28
...
encode simplification_classification, generate(partner_encode) label(simplification_classification)
encode quantity_unit_ortho, generate(udm_encode) label(quantity_unit_ortho)
xi i.year i.partner_encode 
xi i.udm_encode
reg lnprice i.year  i.partner_encode i.udm_encode importexport if geography==19 & grains=="Froment (1)"
...
*merge m:1 year geography using "C:\Users\federico.donofrio\Documents\TOFLIT desktop\Données Stata\priceindex_varioustax_department.dta"
