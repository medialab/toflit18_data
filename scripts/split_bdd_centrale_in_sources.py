...
with open("../base/bdd_centrale.csv", encoding='utf-8') as bdd_centrale:
    ...
    with open('../csv_sources_schema.json', 'r', encoding='utf8') as f:
        ...
        if bdd_fields != flows_fields:
            ...
            if len(missing_fields) > 0:
                print("columns are missing in bdd_centrale.csv: %s" % missing_fields)
            if len(extra_fields) > 0:
                print("Unexpected columns in bdd_centrale.csv: %s" % extra_fields)
    ...
    with open('desagregate_bdd_centrale_in_sources.csv', 'w', encoding='utf8') as report_f:
        report = csv.DictWriter(report_f, fieldnames=["filepath", "nb_line", "diff_nb_line", "sources", "source_types"])
        report.writeheader()
        for (source_type, filename), data in itertools.groupby(data, key=sort_bdd_centrale):
            if filename == "":
            ...
            if WRITE:
                path = os.path.join(SOURCES_ROOT_DIR, 'sources', source_type)
                if not os.path.exists(path):
                    os.makedirs(path)
                with open(real_path, 'w', encoding='utf8') as output:
                    writer = DictWriter(output, fieldnames=flows_headers)
                    writer.writeheader()
                    writer.writerows(source_data)
                datapackage_resource_path.append(filename)
